...
- `rgb 값`에서 0으로 가까워질수록 색이 어두워진다. 검은색에 가까워진다. 50 - 255의 숫자를 랜덤으로 만들어 주기 위한 코드 참고할만하다. 50을 기본값으로 준다. -`개인학습` : 정육면체를 무한대로 만들고 position을 윈도우창의 width/height 크기로 흩뿌려주니, 밤하늘에 별처럼 보인다. 아름답다.
...
- 둘 다 하이라이트/반사광 표현이 잘 되는 재질인데 `Standard`는 `roughness`라는 속성으로 조절한다.
- `Standard`는 `metalness` 금속성을 조절할 수 있다. 어둡게 처리가 된다.
- 성능은 `Phong` 쪽이 좀 더 빠르다.

## flatShading

- 각지게 표현하는 속성으로 `boolean`값으로 부여하면 된다.

## Side

- Object의 구성요소 : 점(Vertex), 선(Edge), 면(Side)
- 카메라를 들고 사각형의 안으로 들어가면, 사라진다. `side: THREE.DoubleSide` 이 속성을 부여하면 정육면체 안으로 들어갈 수 있다. `FrontSide`와 `BackSide`속성이 있는데, FrontSide는 기본값이고 `BackSide`는 뒷면만 보이게 된다.

## 텍스쳐 이미지 로드하기

- 외부 이미지를 가져와서 오브제에 텍스쳐로 붙여보도록 한다. `3d textures` 검색어를 통해 이미지를 다운로드 했다.
- 텍스처 로더 객체를 사용하고 `load()` 메소드에 사용하고자 하는 이미지 경로를 첫 번째 인자로 전달해준다.
- 메소드에 두 번째 인자부터 로드와 관련된 콜백함수를 넣을 수 있다. 순차적으로 로드완료, 로드중, 로드에러에 대한 콜백함수를 사용할 수 있다.
- 그대로 복사할 파일을 사용하고자 하니, 웹팩 설정에서 `new CopyWebpackPlugin`를 변경해줘야한다.
